...
//　デフォルトコンストラクタ
//===============================================================================
CLife::CLife() : CScene(5)//優先優位イニシャライズ
{
	//本体データ
...
{
	/*	m_Life[3].fLength == 現在のライフ					（最前面に来るバー）			*/
	/*	m_Life[2].fLength == ダメージを受けて減った分のバー	（最前面から一つ後ろに来るバー）*/
	//プレイヤーの情報
	CPlayer *pPlayer = CGame::GetPlayer();

	if (pPlayer != NULL)
	{
		if (CPlayer::GetLifeFuture() == true)
		{//スキルを使用した瞬間

			//m_LifeFuture[3]現在のHPと長さを入れる
			m_LifeFuture[3].nLife = m_Life[3].nLife;
			m_LifeFuture[3].fLength = m_Life[3].fLength;

			//m_LifeFuture[2]現在のHPと長さを入れる
			m_LifeFuture[2].nLife = m_Life[2].nLife;
			m_LifeFuture[2].fLength = m_Life[2].fLength;
			
			CPlayer::SetLifeFuture(false);
		}

		if (CPlayer::GetLifeFutureSukill() == true)
		{//スキルをキャンセルした瞬間

			//m_Life[3]過去のHPと長さを入れる
			m_Life[3].nLife = m_LifeFuture[3].nLife;
			// 回復したとき後ろのバーも増やす
			m_Life[3].fLength = m_LifeFuture[3].fLength;

			//m_Life[2]過去のHPと長さを入れる
			m_Life[2].nLife = m_LifeFuture[2].nLife;
			// 回復したとき後ろのバーも増やす
			m_Life[2].fLength = m_LifeFuture[2].fLength;
			...
			//頂点座標の設定
			pVtx[(2 * 4) + 1].pos = D3DXVECTOR3(m_Life[2].pos.x + m_Life[2].fLength + HP_DIAMOND, m_Life[2].pos.y, 0.0f);
			pVtx[(2 * 4) + 3].pos = D3DXVECTOR3(m_Life[2].pos.x + m_Life[2].fLength, m_Life[2].pos.y + BAR_HEIGHT_LIFE, 0.0f);

			//頂点座標の設定
			pVtx[(3 * 4) + 1].pos = D3DXVECTOR3(m_Life[3].pos.x + m_Life[3].fLength + HP_DIAMOND, m_Life[3].pos.y, 0.0f);
			pVtx[(3 * 4) + 3].pos = D3DXVECTOR3(m_Life[3].pos.x + m_Life[3].fLength, m_Life[3].pos.y + BAR_HEIGHT_LIFE, 0.0f);

			// 頂点バッファをアンロックする
			m_pVtxBuff->Unlock();

			CPlayer::SetLifeFutureSukill(false);
		}
...
void CLife::MakeVertexLife(LPDIRECT3DDEVICE9 pDevice)
{
	...
	{
		//頂点座標の設定
		pVtx[(nCntLife * 4) + 0].pos = D3DXVECTOR3(m_Life[nCntLife].pos.x + HP_DIAMOND, m_Life[nCntLife].pos.y, 0.0f);
		pVtx[(nCntLife * 4) + 1].pos = D3DXVECTOR3(m_Life[nCntLife].pos.x + BAR_WIDTH_LIFE + HP_DIAMOND, m_Life[nCntLife].pos.y, 0.0f);
