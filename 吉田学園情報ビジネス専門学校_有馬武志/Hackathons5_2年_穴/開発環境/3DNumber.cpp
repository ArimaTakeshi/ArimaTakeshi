...
void C3DNumber::Set(D3DXVECTOR3 pos, float fLengthX, float flengthY, int nScore, int nDigits, D3DXCOLOR col, STATE state)	//スコア(数字)のTEXTURE
{
	...
	{//生成できた！
		...
		{//桁数分回転
			...
			if (m_p3D != NULL)
			{//生成できた！
				m_p3D[nCount]->Set(pos + D3DXVECTOR3(nCount * (fLengthX * -2.0f), 0.0f, 0.0f),D3DXVECTOR3(0.0f, 0.0f, 0.0f),col, D3DXVECTOR3(fLengthX, flengthY, 0.0f), nNumber % 10, NUMBER_UV_X, NUMBER_UV_Y, CScene::DRAW_TYPE_NORMAL, CScene::TEX_MAX, true);
