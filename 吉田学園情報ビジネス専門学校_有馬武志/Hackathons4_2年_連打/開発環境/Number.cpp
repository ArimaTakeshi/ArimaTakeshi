...
void CNumber::Set(D3DXVECTOR3 pos, float fLengthX, float flengthY, int nScore, int nDigits, D3DXCOLOR col, STATE state)	//スコア(数字)のTEXTURE
{
	...
	{//生成できた！
		...
		{//桁数分回転
			...
			if (m_p2D != NULL)
			{//生成できた！
				m_p2D[nCount]->Set(pos + D3DXVECTOR3(nCount * (fLengthX * -2.0f), 0.0f, 0.0f), fLengthX, flengthY, D3DX_PI, nNumber % 10, NUMBER_UV_X, NUMBER_UV_Y, col, C2D::TEXTURE_MAX);
				m_p2D[nCount]->BindTexture(C2D::Load(C2D::TEX_SCORE));	//画像貼り付け
			}
