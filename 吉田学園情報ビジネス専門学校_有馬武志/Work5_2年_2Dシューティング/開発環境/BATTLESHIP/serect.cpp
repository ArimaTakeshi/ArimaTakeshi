...
// マクロ定義
//*****************************************************************************
#define TEXTURE_NAME_0			"data\\TEXTURE\\serect.png"		//読み込むテクスチャファイル
...
//　デフォルトコンストラクタ
//===============================================================================
CSerect::CSerect(int nPriority) : CScene(nPriority)
{
	m_pVtxBuff = NULL;
...
void CSerect::Update(void)
{
	...
	if (pCInputKeyBoard->GetKeyboardTrigger(DIK_UP) == true || pCInputKeyBoard->GetKeyboardTrigger(DIK_W) == true)
	{//矢印キーの上を押したとき

		//ポーズの選択し移動音
		pSound->PlaySound(CSound::SOUND_LABEL_PAUSE002);
	...
	if (pCInputKeyBoard->GetKeyboardTrigger(DIK_DOWN) == true || pCInputKeyBoard->GetKeyboardTrigger(DIK_S) == true)
	{//矢印キーの下を押したとき
	...
	if (pCInputKeyBoard->GetKeyboardTrigger(DIK_RETURN) == true || pCInputMouse->GetMouseTrigger(0) == true)
	{
		if (m_SelectNum == 0)
		{
			//ゲームの最初に戻る
