...
#ifdef _DEBUG
	// デバッグ情報表示用フォントの生成
	//D3DXCreateFont(m_pD3DDevice, 100, 40, 0, 0, FALSE, MONO_FONT,
...
{/*
	...
	wsprintf(str, "My name is Arimaｱﾘﾏ:%d\n", Fps);
	wsprintf(Str, "お給料ぷりーず:%d\n", Fps * 60);

	// テキスト描画
	m_pFont->DrawText(NULL, str, -1, &rect, DT_CENTER, D3DCOLOR_ARGB(0xff, 255, 255, 255));
	rect = { 0, 200, SCREEN_WIDTH, SCREEN_HEIGHT };
	m_pFont->DrawText(NULL, Str, -1, &rect, DT_CENTER, D3DCOLOR_ARGB(0xff, 255, 100, 80));
	*/
}
#endif
////=============================================================================
