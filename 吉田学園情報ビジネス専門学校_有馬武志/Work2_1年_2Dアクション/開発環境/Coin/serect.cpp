...
#define SERECT_TEXTURE_NAME1	"data\\TEXTURE\\tutorial2.png"		//読み込むテクスチャファイル
#define SERECT_TEXTURE_NAME2	"data\\TEXTURE\\ランキング.png"		//読み込むテクスチャファイル
#define MAX_TEXTURE				(4)									//テクスチャの最大数
#define SERECT_POS_X			(0)									//セレクトの左上X座標
...
void InitSerect(void)
{
	...
	//セレクトの座標
	float PosSerect = (SCREEN_HEIGHT / 2 - 271.0f)  ;
	...
	{//全テクスチャのカウント
		...
		{//それ以外の場合
			//頂点設定
			pVtx[0].pos = D3DXVECTOR3(SERECT_WIDTH / 2 - 200, PosSerect, 0.0f);				//左上
			pVtx[1].pos = D3DXVECTOR3(SERECT_WIDTH / 2 + 200, PosSerect, 0.0f);				//右上
			pVtx[2].pos = D3DXVECTOR3(SERECT_WIDTH / 2 - 200, PosSerect + 150, 0.0f);		//左下
			pVtx[3].pos = D3DXVECTOR3(SERECT_WIDTH / 2 + 200, PosSerect + 150, 0.0f);		//右下
			if (nCount == 1)
			{
				//頂点カラー設定
				pVtx[0].col = D3DCOLOR_RGBA(255, 255, 255, 255);
...
void UpdateSerect(void)
{
	...
	{//矢印キーの上を押したとき
		...
		g_Serect.SelectNum = (g_Serect.SelectNum + 2) % 3;
		g_Serect.SelectColor = 1.0f;
	}
	...
	{//矢印キーの下を押したとき
		...
		g_Serect.SelectNum = (g_Serect.SelectNum + 1) % 3;
		g_Serect.SelectColor = 1.0f;
	}

	//点滅処理
	//選択しているカラーの値を減らしていく
	//g_Serect.SelectColor -= 0.004f;
