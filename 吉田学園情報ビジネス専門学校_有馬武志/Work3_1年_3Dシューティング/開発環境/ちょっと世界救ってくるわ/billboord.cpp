...
void DeleteBillBoord(int nCntBillBoord)
{
	if (g_BillBoord[nCntBillBoord].nType == 4)
	{
		...
		// 地面に血を出す
		SetBlood(D3DXVECTOR3(g_BillBoord[nCntBillBoord].posBillBoord.x, 0.0f, g_BillBoord[nCntBillBoord].posBillBoord.z), D3DXVECTOR3(g_BillBoord[nCntBillBoord].rotBillBoord.x, g_BillBoord[nCntBillBoord].rotBillBoord.y - (rand() % 30), g_BillBoord[nCntBillBoord].rotBillBoord.z), 20.0f + (rand() % 10), 20.0f + (rand() % 10));

		//killのスコア
		AddScore(1,SCORE_KILL);
	}
}
//=========================================================================================================================
// ビルボードのあたり判定
