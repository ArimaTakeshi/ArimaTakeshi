...
#define WINDOW_NAME		"ちょっと世界救ってくるわ"	// ウインドウのキャプション名
...
MODE g_mode = MODE_TITLE;					//タイトルから
//デバック用
//MODE g_mode = MODE_GAME;						//ゲームから
//MODE g_mode = MODE_RESULT;					//リザルトから
//MODE g_mode = MODE_RANKING;					//ランキングから
...
void SetMode(MODE mode)
{
	...
	case MODE_RESULT:	//リザルト
		UninitResult();
		StopSound(SOUND_LABEL_COMBATBGM000);
		StopSound(SOUND_LABEL_PLANEBGM001);
		StopSound(SOUND_LABEL_PLANEBGM003);
		break;
	case MODE_RANKING:	//ランキング
		UninitRanking();
		break;
	...
	case MODE_GAME:		//ゲーム
		InitGame();
		PlaySound(SOUND_LABEL_COMBATBGM000);
