...
#define MAX_POLYGON			(62)							//スコアの最大桁数
#define MAX_TEXTURE			(2)								//スコアの最大桁数
#define MAX_LENGTH_TEX		(60)							//スコアの最大桁数
#define MAX_LENGTH_NUM		(2)								//スコアの最大桁数

#define LENGTH_SIZE			(15)
#define TEXLENGTH_SIZE_X	(5)
...
void InitLength(void)
{
	...
	//数値
	g_posLength = D3DXVECTOR3(190, 35, 0.0f);

	//ゲージ
	g_posTexLength = D3DXVECTOR3(240, 35, 0.0f);
...
void MakeVertexLength(LPDIRECT3DDEVICE9 pDevice)
{
	...
	for (nCntLength = 0; nCntLength < MAX_POLYGON; nCntLength++)
	{
		...
		else if (nCntLength >= MAX_POLYGON - 55)
		{
			//頂点カラー
