...
void UpdateBlood(void)
{
	...
	for (int nCntBlood = 0; nCntBlood < BLOOD_MAX; nCntBlood++)
	{
		if (g_aBlood[nCntBlood].bUse == true)
		{
			...
			//頂点情報の設定
			pVtx[0].pos = D3DXVECTOR3(-(g_aBlood[nCntBlood].fHeight), 0.0f, (g_aBlood[nCntBlood].fWide));		//頂点座標(X座標,Y座標)
			pVtx[1].pos = D3DXVECTOR3((g_aBlood[nCntBlood].fHeight), 0.0f, (g_aBlood[nCntBlood].fWide));		//頂点座標(X座標,Y座標)
			pVtx[2].pos = D3DXVECTOR3(-(g_aBlood[nCntBlood].fHeight), 0.0f, -(g_aBlood[nCntBlood].fWide));	//頂点座標(X座標,Y座標)
			pVtx[3].pos = D3DXVECTOR3((g_aBlood[nCntBlood].fHeight), 0.0f, -(g_aBlood[nCntBlood].fWide));		//頂点座標(X座標,Y座標)

			pVtx += 4;

			//g_nWBlood = g_nWBlood + fCntBlood;
		}
...
void MakeVertexBlood(LPDIRECT3DDEVICE9 pDevice)
{
	...
	for (int nCntBlood = 0; nCntBlood < BLOOD_MAX; nCntBlood++)
	{
		...
		//頂点カラーの設定
		pVtx[0].col = D3DXCOLOR(0.6f, 0.0f, 0.0f, 1.0f);  //色の指定（0.0f ～ 1.0f）&透明度（0.0f　～　1.0f）
		pVtx[1].col = D3DXCOLOR(0.6f, 0.0f, 0.0f, 1.0f);  //色の指定（0.0f ～ 1.0f）&透明度（0.0f　～　1.0f)
		pVtx[2].col = D3DXCOLOR(0.6f, 0.0f, 0.0f, 1.0f);  //色の指定（0.0f ～ 1.0f）&透明度（0.0f　～　1.0f）
		pVtx[3].col = D3DXCOLOR(0.6f, 0.0f, 0.0f, 1.0f);  //色の指定（0.0f ～ 1.0f）&透明度（0.0f　～　1.0f）
...
int SetBlood(D3DXVECTOR3 pos, D3DXVECTOR3 rot, float fHeight, float fWide)
{
	...
	for (nCntBlood = 0; nCntBlood < BLOOD_MAX; nCntBlood++)
	{
		if (g_aBlood[nCntBlood].bUse == false)
		{
			...
			g_aBlood[nCntBlood].fHeight = fHeight;
			g_aBlood[nCntBlood].fWide = fWide;
			g_aBlood[nCntBlood].bUse = true;

			//頂点情報の設定
			pVtx[0].pos = D3DXVECTOR3(pos.x - fWide, 0.0f, pos.z + fHeight);		//頂点座標(X座標,Y座標)
			pVtx[1].pos = D3DXVECTOR3(pos.x + fWide, 0.0f, pos.z + fHeight);		//頂点座標(X座標,Y座標)
			pVtx[2].pos = D3DXVECTOR3(pos.x - fWide, 0.0f, pos.z - fHeight);		//頂点座標(X座標,Y座標)
			pVtx[3].pos = D3DXVECTOR3(pos.x + fWide, 0.0f, pos.z - fHeight);		//頂点座標(X座標,Y座標)
